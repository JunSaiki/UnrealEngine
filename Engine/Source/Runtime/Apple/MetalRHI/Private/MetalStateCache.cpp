		...
		if (RenderTargetsInfo.DepthStencilRenderTarget.Texture != nullptr)
		{
			...
			if (StencilTexture && !bDepthStencilSampleCountMismatchFixup && bUsingStencil)
			{
                ...
                else
                {
                    ...
                    NewStencilStore = GetMetalRTStoreAction(HighLevelStoreAction);
...
void FMetalStateCache::SetVertexStream(uint32 const Index, id<MTLBuffer> Buffer, NSData* Bytes, uint32 const Offset, uint32 const Length)
{
	...
	check(CommandEncoder.IsRenderCommandEncoderActive())
	{
		...
		if (bConditionalSwitch && bSupportsDeferredStore)
		{
			for (int32 RenderTargetIndex = 0; RenderTargetIndex < RenderTargetsInfo.NumColorRenderTargets; RenderTargetIndex++)
			{
				...
				{
					const bool bMultiSampled = RenderTargetView.Texture->GetNumSamples() > 1;
			...
			{
				const bool bMultiSampled = RenderTargetsInfo.DepthStencilRenderTarget.Texture->GetNumSamples() > 1;
