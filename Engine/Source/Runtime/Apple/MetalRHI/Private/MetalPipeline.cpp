...
struct FMetalGraphicsPipelineKey
{
	...
	friend void InitMetalGraphicsPipelineKey(FMetalGraphicsPipelineKey& Key, const FGraphicsPipelineStateInitializer& Init, EMetalIndexType const IndexType)
	{
		...
		check(NumActiveTargets <= MaxMetalRenderTargets);
...
static FMetalShaderPipeline* CreateMTLRenderPipeline(const FGraphicsPipelineStateInitializer& Init, EMetalIndexType const IndexType)
{
	...
	if(PixelShader && (PixelShader->Bindings.InOutMask & 0x8000) && RenderPipelineDesc.depthAttachmentPixelFormat == MTLPixelFormatInvalid)
	{
	...
	RenderPipelineDesc.sampleCount = Init.NumSamples;
#if PLATFORM_MAC
	...
	else
	{
		...
		UE_CLOG((Pipeline.ComputePipelineState == nil), LogMetal, Error, TEXT("Vertex shader: %s"), *FString(VertexShader->GlslCodeNSString));
		UE_CLOG((Pipeline.ComputePipelineState == nil), LogMetal, Error, TEXT("Pixel shader: %s"), PixelShader ? *FString(PixelShader->GlslCodeNSString) : TEXT("NULL"));
		UE_CLOG((Pipeline.ComputePipelineState == nil), LogMetal, Error, TEXT("Hull shader: %s"), *FString(HullShader->GlslCodeNSString));
		UE_CLOG((Pipeline.ComputePipelineState == nil), LogMetal, Error, TEXT("Domain shader: %s"), *FString(DomainShader->GlslCodeNSString));
	...
	UE_CLOG((Pipeline.RenderPipelineState == nil), LogMetal, Error, TEXT("Vertex shader: %s"), *FString(VertexShader->GlslCodeNSString));
	UE_CLOG((Pipeline.RenderPipelineState == nil), LogMetal, Error, TEXT("Pixel shader: %s"), PixelShader ? *FString(PixelShader->GlslCodeNSString) : TEXT("NULL"));
	UE_CLOG((Pipeline.RenderPipelineState == nil), LogMetal, Error, TEXT("Hull shader: %s"), HullShader ? *FString(HullShader->GlslCodeNSString) : TEXT("NULL"));
	UE_CLOG((Pipeline.RenderPipelineState == nil), LogMetal, Error, TEXT("Domain shader: %s"), DomainShader ? *FString(DomainShader->GlslCodeNSString) : TEXT("NULL"));
...
#if METAL_DEBUG_OPTIONS
	Pipeline.ComputeSource = DomainShader ? VertexShader->GlslCodeNSString : nil;
	Pipeline.VertexSource = DomainShader ? DomainShader->GlslCodeNSString : VertexShader->GlslCodeNSString;
	Pipeline.FragmentSource = PixelShader ? PixelShader->GlslCodeNSString : nil;
#endif
	...
	if (GFrameCounter > 3)
	{
		...
		NSLog(@"\nDomainShader:");
		NSLog(@"%@", DomainShader ? nDomainShader->GlslCodeNSString : @"NONE");
		NSLog(@"===============================================================");
	}
#endif
