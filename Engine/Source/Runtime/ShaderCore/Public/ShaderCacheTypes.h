...
enum EShaderCacheStateValues
{
	...
	EShaderCacheMaxNumResources = 64,
...
struct SHADERCORE_API FShaderDrawKey
{
	...
	uint64 UsedResources[SF_NumFrequencies];
	uint8 IndexType;
	mutable uint32 Hash;
	...
	friend bool operator ==(const FShaderDrawKey& A,const FShaderDrawKey& B)
	{
		bool Compare = A.IndexType == B.IndexType && 
			FMemory::Memcmp(&A.UsedResources, &B.UsedResources, sizeof(A.UsedResources)) == 0 &&
	...
	friend uint32 GetTypeHash(const FShaderDrawKey &Key)
	{
		if(!Key.Hash)
		{
			...
			for( uint32 i = 0; i < SF_NumFrequencies; i++ )
			{
				...
				uint32 NumResources = (uint32)FMath::FloorLog2_64(Key.UsedResources[i]);
	...
	friend FArchive& operator<<( FArchive& Ar, FShaderDrawKey& Info )
	{
		...
		for ( uint32 i = 0; i < SF_NumFrequencies; i++ )
		{
			...
			uint32 NumResources = (uint32)FMath::FloorLog2_64(Info.UsedResources[i]);
			Ar << NumResources;
			Info.UsedResources[i] = NumResources;
			for ( uint32 j = 0; j < NumResources; j++ )
			{
